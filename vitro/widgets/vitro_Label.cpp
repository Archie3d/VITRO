...
void Label::update()
{
    ...
    if (const auto&& [changed, align]{ getStylePropertyChanged(attr::css::text_align) }; changed) {
        juce::Label::setJustificationType(align.isVoid() ? Justification::left
                                                         : parseJustificationFromString(align.toString()));
    }
}
